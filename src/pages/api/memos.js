...
//         const decoded = jwt.verify(token, 'your_secret_key'); // 비밀키 확인
...
//     try {
//         if (req.method === 'GET') {
//             const token = req.headers.authorization?.replace('Bearer ', ''); // 옵셔널 체이닝 연산자 사용
...
async function getUserData(token) {
    try {
        ...
        const decoded = jwt.verify(token, 'your_secret_key'); // Verify the secret key
        const userId = decoded.userId;
